...
export default function PaymentModal({ transaction, onClose, onConfirm }: PaymentModalProps) {
  ...
  useEffect(() => {
    ...
    if (qe && transaction?._id) {
      (async () => {
        try {
          const r = await fetch(`http://103.236.194.99:3000/payment/qpay?id=${transaction._id}&amount=${qe.dun}`);
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mt-0.5">
                  <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
                    ...
                    <span className=" uppercase tracking-wide">{transaction.mashiniiDugaar}</span>
                  </div>
                  {duration && (
