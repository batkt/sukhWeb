    ...
    <div className="min-h-0 flex flex-col p-4 sm:p-6 md:p-8">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[color:var(--muted-text)]">Мэдээ, мэдээлэл болон нийтлэл удирдах</p>
          </div>
        </div>
      ...
      >
        <div className="flex flex-col gap-5 pt-4">
          ...
          <div>
            <label className="block text-sm  text-[color:var(--panel-text)] mb-1.5 ml-1">Гарчиг</label>
            <Input
              placeholder="Нийтлэлийн гарчиг..."
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              className="h-11 rounded-xl bg-[color:var(--surface-hover)] dark:bg-white/5 border-0 focus:ring-2 focus:ring-theme/30"
            />
          </div>
          
          <div>
            <label className="block text-sm  text-[color:var(--panel-text)] mb-1.5 ml-1">Агуулга</label>
            <Input.TextArea
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5 ml-1">
              <label className="block text-sm text-[color:var(--panel-text)]">Зураг</label>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {existingImages.map((url, idx) => (
                <div key={`existing-${idx}`} className="relative group w-20 h-20">
                  <img src={url} alt="" className="w-full h-full object-cover rounded-xl border border-[color:var(--surface-border)] dark:border-white/10" />
                  <button
                    onClick={() => setExistingImages(p => p.filter((_, i) => i !== idx))}
                    className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-danger text-white flex items-center justify-center shadow-lg transition-transform hover:scale-110"
                  >
                    <X size={14} />
                  </button>
                </div>
