...
const fetcherJagsaalt = async ([
  ...
  try {
    ...
    const branchAware = orgOnly.filter((it: Khariltsagch) => {
...
export function useKhariltsagchJagsaalt(
  token: string,
  baiguullagiinId: string | number,
  query: Query = {},
  barilgiinId?: string | number | null
) {
  const [khuudaslalt, setKhariltsagchKhuudaslalt] = useState<Khuudaslalt>({
    khuudasniiDugaar: 1,
  ...
  const { data, mutate, isValidating, error }: SWRResponse<any, any> = useSWR(
    shouldFetch
      ? ["/khariltsagch", token, baiguullagiinId, barilgiinId, khuudaslalt, query]
      : null,
    fetcherJagsaalt,
    {
  ...
  return {
    KhariltsagchGaralt: data,
    KhariltsagchJagsaaltMutate: mutate,
...
export default useKhariltsagchJagsaalt;
